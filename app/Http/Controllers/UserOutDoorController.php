...
class UserOutDoorController extends Controller
{
    ...
    public function ragapayNotifications(Request $request)
    {
        ...
        if(strtolower($data['type']) == 'sale' && $data['status'] == 'success' && $deposit->status == 'Pending') {
            $respTrans = $this->performTransaction($data['order_currency'], $t7->number, $amount, 'MM-Ragapay', 'MM-AUTORP-'.$txn_id, 'deposit', 'balance');
    ...
    public function handlePaycly(Request $request)
    {
        ...
        if($deposit) {
            if($data['status'] == 'Failed') {
                ...
                $respTrans = $this->performTransaction($t7->currency, $t7->number, $amount, 'MM-Paycly', 'MM-AUTOPAYCLY-'.$paymentId, 'deposit', 'balance');
    ...
    public function xproNotifications(Request $request)
    {
        ...
        if(strtolower($data['type']) == 'sale' && $data['status'] == 'success' && $deposit->status == 'Pending') {
            $respTrans = $this->performTransaction($data['order_currency'], $t7->number, $amount, 'MM-Xpro', 'MM-AUTOXPRO-'.$txn_id, 'deposit', 'balance');
